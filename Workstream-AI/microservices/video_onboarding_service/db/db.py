...
def init_db():
    ...
    try:
        cur.execute("ALTER TABLE video_sessions ADD COLUMN intake_data TEXT")
    except sqlite3.OperationalError as e:
        if "duplicate column name" not in str(e).lower() and "already exists" not in str(e).lower():
            print(f"Migration info (intake_data): {e}")
...
def _insert_predefined_questions(cur: sqlite3.Cursor):
    ...
    questions = [
        (101, "Can you tell me, in your own words, what happened and when it began?", "audio", category, True, 1, None, 180),
        (102, "Where did the incident happen, and is that place safe for you now?", "audio", category, True, 2, None, 120),
        (103, "Who was involved, and do you know their current location or relationship to you?", "audio", category, True, 3, None, 120),
        (104, "Have you already reported this to the police, a court, or any authority? What happened next?", "audio", category, True, 4, None, 180),
        (105, "Are you or anyone with you in immediate danger or in need of medical help right now?", "audio", category, True, 5, None, 120),
        (106, "What kind of legal, safety, shelter, medical, or financial support would help you most today?", "audio", category, True, 6, None, 180),
        (107, "Do you have any evidence, documents, messages, photos, or witnesses that you want an officer to know about?", "audio", category, True, 7, None, 180),
        (108, "Is there anything about your family, children, disability, or identity that should shape the support we arrange?", "audio", category, True, 8, None, 180),
        (109, "Who is a trusted person we may contact, and what is the safest way and time to reach you?", "audio", category, True, 9, None, 120),
        (110, "What is the most important thing you want SAHAY to do after this conversation?", "audio", category, True, 10, None, 120),
    ]
...
def create_video_session(employee_name: str, employee_id: str, employee_email: Optional[str] = None, intake_data: Optional[dict] = None) -> dict:
    ...
    cur.execute("""
        ...
        (session_id, employee_name, employee_id, employee_email, intake_data, meet_link, jitsi_room_id, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, 'created')
    """, (session_id, employee_name, employee_id, employee_email, json.dumps(intake_data or {}), meet_link, jitsi_room_id))
